...
class Temple extends React.Component<any,state>{
    constructor(props:{}){
        super(props);
        this.state = {
            temple : {id:'',name:'',ico:''},
    ...
    componentWillMount(){
        ...
        this.props.getTempleMessage({id,ifset:true}, (res:any) =>{
            this.setState({
                ...res,
                temple : res.temple[0]
            })
    ...
    private handleClickPray(id:string ,e: React.MouseEvent<HTMLDivElement>){
        e.preventDefault()
    ...
    render(){
        let { temple, facility, templeMaterial} = this.state

        let rowData = R.splitEvery(2)(facility)
        return (
            <div>
                <WhiteSpace />
...
export default connect( ()=>({}) ,
    dispatch => ({
        getTempleMessage: (payload:payload, callback:()=>void) => dispatch({type: TO_GET_TEMPLE, payload, callback}),
